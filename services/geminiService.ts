...
# ГЛАВНЫЙ ПРИНЦИП
Используй только данные из предоставленного рецепта. Расчёт КБЖУ является обязательным на основе ингредиентов. Для остальных полей, если информации нет, оставляй их пустыми, но не выдумывай данные.
...
1.  **Номер**:
    *   Найди номер рецепта в исходном тексте (обычно он выглядит как "Рецепт № ...").
    *   Запиши только сам номер. Например, если в тексте "Рецепт № 277", в это поле запиши "277".
    *   Если номер не найден, оставь поле пустым "".

2.  **Заголовок**:
    *   Найди название блюда в тексте и запиши его сюда. Это будет заголовок поста.
    *   Если явного названия нет, придумай короткий и аппетитный заголовок на основе ингредиентов.

3.  **Рецепт**:
    *   Напиши готовый текст рецепта простым, тёплым, спокойным языком. Текст должен быть готов к публикации в Instagram.
    *   **Не добавляй приветствий** (например, "Привет!" или "Сегодня у нас..."). Сразу начинай с описания блюда.
    *   **Используй эмодзи** 🥐✨, чтобы сделать текст более живым и аппетитным.
    ...
    *   Текст должен быть без специального форматирования (никакого **bold** или *italic*). Только простой текст и эмодзи.

4.  **Совет**:
    *   Сюда запиши полезный совет, лайфхак или призыв к действию (например, 'Сохрани рецепт ❤️'), который должен идти в конце поста.

5.  **ДопИнфа**:
    *   Обязательно рассчитай КБЖУ (калории, белки, жиры, углеводы) на одну порцию, основываясь на списке ингредиентов и их количестве из рецепта.
    *   Формат: "КБЖУ на 1 порцию: ~ К: ... ккал, Б: ... г, Ж: ... г, У: ... г".
    *   Твоя задача — сделать наилучший возможный расчёт. Недопустимо отвечать, что данных недостаточно.
...
8.  **Промпт**:
    ...
    *   Основывайся на поле "Заголовок" и внешнем виде готового блюда из рецепта.
